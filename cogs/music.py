        ...
        if m.activities:
            for activity in m.activities:
                if isinstance(activity, discord.Spotify):
                    ...
                    await interaction.response.send_message(embed = embed, view = view, ephemeral = True if member else False)
                    return
                    ...
                    if activity.name == 'YouTube Music':
                        if activity.small_image_text != "Playing":
                            ...
                            return
                        embed = discord.Embed(title = f"{f'{m.display_name}が' if member else ''}{activity.name}で再生中", description = "これ以上の情報は不明です。", color = discord.Colour(0x1DB954))
                        ...
                        await interaction.response.send_message(embed = embed, ephemeral = True if member else False)
                        return
                    elif activity.name == 'Spotify':
                        embed = discord.Embed(title = f"{f'{m.display_name}が' if member else ''}{activity.name}で再生中", description = "これ以上の情報は不明です。\n-# おそらくローカルファイルを再生中です。", color = discord.Colour(0x1DB954))
                        embed.add_field(name = "タイトル", value = activity.details, inline = False)
                        if activity.state:
                            embed.add_field(name = "詳細",    value = activity.state, inline = False)
                        embed.add_field(name = "再生開始時間", value = f"{discord.utils.format_dt(activity.start, 'T')}", inline = False)
